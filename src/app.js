...
function renderDrawer(lead) {
  ...
  const accordion = (title, content, expanded = false) => `<section class="accordion ${expanded ? 'open' : ''}"><button class="accordion-trigger" type="button" data-accordion aria-expanded="${expanded}"><span>${title}</span><span class="accordion-icon">⌄</span></button><div class="accordion-content"><div class="details">${content}</div></div></section>`;
...
function bind() {
  ...
  document.querySelectorAll('[data-accordion]').forEach(el => el.addEventListener('click', () => { const accordion = el.closest('.accordion'); const isOpen = accordion.classList.toggle('open'); el.setAttribute('aria-expanded', String(isOpen)); }));
