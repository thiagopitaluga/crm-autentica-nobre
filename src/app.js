...
const whatsappIcon = '<svg class="whatsapp-icon" viewBox="0 0 32 32" aria-hidden="true"><path fill="currentColor" d="M16 3a13 13 0 0 0-11.1 19.8L3 29l6.4-1.7A13 13 0 1 0 16 3Zm0 23.7c-2 0-4-.5-5.7-1.6l-.4-.2-3.8 1 1-3.7-.3-.4A10.6 10.6 0 1 1 16 26.7Zm5.8-7.9c-.3-.2-1.9-.9-2.2-1s-.5-.2-.7.2-.8 1-.9 1.2-.3.3-.6.1a8.7 8.7 0 0 1-2.6-1.6 9.8 9.8 0 0 1-1.8-2.2c-.2-.3 0-.5.1-.6l.5-.5.3-.5c.1-.2 0-.4 0-.6l-1-2.3c-.2-.5-.5-.4-.7-.4h-.6c-.2 0-.6.1-.9.5s-1.2 1.1-1.2 2.8 1.3 3.3 1.5 3.5a12.1 12.1 0 0 0 4.7 4.2c.6.3 1 .5 1.4.6.6.2 1.2.2 1.6.1.5-.1 1.9-.8 2.2-1.6s.3-1.5.2-1.6-.2-.2-.5-.4Z"/></svg>';
...
function renderLead(lead, compact = false) {
  ...
  const whatsapp = `<button class="whatsapp" data-whatsapp="${esc(leadId(lead))}" ${phone ? '' : 'disabled'}>${phone ? `${whatsappIcon} Chamar no WhatsApp` : 'Telefone não informado'}</button>`;
...
function renderDrawer(lead) {
  ...
  const leadData = `${read('WhatsApp',value(lead,'número_do_whatsapp'))}${read('E-mail',value(lead,'email'))}${read('Interesse',displayInterest(lead))}${read('Resposta sobre parcela',budget(lead))}${read('Origem',origin(lead))}${read('Criação',formatDate(value(lead,'created_time')))}`;
  const campaignData = `${read('Campanha',value(lead,'campaign_name'))}${read('Conjunto',value(lead,'adset_name'))}${read('Anúncio',value(lead,'ad_name'))}${read('Formulário',value(lead,'form_name'))}${read('Status Meta',value(lead,'lead_status'))}`;
  const accordion = (title, content, expanded = false) => `<section class="accordion ${expanded ? 'open' : ''}"><button class="accordion-trigger" type="button" data-accordion aria-expanded="${expanded}"><span class="accordion-title"><b>${title}</b><small class="accordion-state">${expanded ? 'Ocultar dados' : 'Ver dados'}</small></span><span class="accordion-icon">⌄</span></button><div class="accordion-content"><div class="details">${content}</div></div></section>`;
  return `<div class="overlay" id="close-drawer"></div><aside class="drawer"><div class="drawer-top"><button class="drawer-close" id="drawer-close">×</button><p class="eyebrow">DETALHES DO LEAD</p><h2>${esc(nameOf(lead))}</h2><button class="whatsapp drawer-whatsapp" data-whatsapp="${esc(leadId(lead))}" ${normalizePhone(value(lead, 'número_do_whatsapp')) ? '' : 'disabled'}>${normalizePhone(value(lead, 'número_do_whatsapp')) ? `${whatsappIcon} Chamar no WhatsApp` : 'Telefone não informado'}</button></div><form id="crm-form" class="drawer-scroll">${accordion('Dados do lead', leadData, true)}${accordion('Dados da campanha', campaignData)}<section class="drawer-section commercial-section"><h3>Atendimento comercial</h3><label>Status CRM<select name="status">${CRM_STATUSES.map(s=>`<option ${crm.status===s?'selected':''}>${s}</option>`).join('')}</select></label>${field('Responsável','owner')}${field('Data do primeiro contato','firstContact','datetime-local')}${field('Último contato','lastContact','datetime-local')}${field('Data da visita','visitDate','datetime-local')}<label>Resultado<input name="result" value="${esc(crm.result || '')}"></label><label>Observações<textarea name="notes">${esc(crm.notes || '')}</textarea></label></section></form><div class="drawer-footer"><button class="save" type="submit" form="crm-form">Salvar atendimento</button></div></aside>`;
}
...
function bind() {
  ...
  document.querySelectorAll('[data-accordion]').forEach(el => el.addEventListener('click', () => { const accordion = el.closest('.accordion'); const isOpen = accordion.classList.toggle('open'); el.setAttribute('aria-expanded', String(isOpen)); el.querySelector('.accordion-state').textContent = isOpen ? 'Ocultar dados' : 'Ver dados'; }));
